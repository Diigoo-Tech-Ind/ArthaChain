...
impl SmartContractsService {
    ...
    pub async fn get_contract_by_address(&self, address: &str) -> Result<ContractInfo, String> {
        ...
        // Decode hex address
        let address_bytes =
            hex::decode(address).map_err(|_| "Invalid address format".to_string())?;
    ...
    pub async fn deploy_contract(
        ...
        // Decode bytecode
        let bytecode =
            hex::decode(&request.bytecode).map_err(|_| "Invalid bytecode format".to_string())?;
    ...
    pub async fn call_contract(
        ...
        let return_value = Some(format!(
            "0x{}",
            hex::encode(format!("result_{}", request.function_name))
        ));
