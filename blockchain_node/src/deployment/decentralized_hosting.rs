...
impl DecentralizedHostingManager {
    ...
    async fn initialize_p2p_network(&self) -> Result<()> {
        ...
        log::info!("✅ P2P network initialized with {} max peers", self.config.p2p_config.max_peers);
        Ok(())
    }
    ...
    async fn setup_community_nodes(&self) -> Result<()> {
        ...
        log::info!("✅ {} community nodes configured", self.config.community_nodes.len());
        Ok(())
    }
    ...
    pub async fn get_active_nodes(&self) -> Vec<NodeStatus> {
        ...
        nodes.values().filter(|node| node.is_active).cloned().collect()
    }
...
fn generate_node_id() -> String {
    ...
    for _ in 0..19 { // 19 chars to make total 21 with "ArthaX" prefix
